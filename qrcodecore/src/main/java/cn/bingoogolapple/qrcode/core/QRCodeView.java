...
public abstract class QRCodeView extends RelativeLayout implements Camera.PreviewCallback, ProcessDataTask.Delegate {
    ...
    public void startCamera(int cameraFacing) {
        if (cameraFacing==0){
            Log.i(TAG, "for后置 "+cameraFacing);
        }else if (cameraFacing==1){
            Log.i(TAG, "for前置 "+cameraFacing);
        }
        Log.i(TAG, "startCamera "+cameraFacing);
       /* if (mCamera != null) {
            return;
        }*/
        ...
        for (int cameraId = 0; cameraId < Camera.getNumberOfCameras(); cameraId++) {
            ...
            if (cameraInfo.facing == cameraFacing) {
                Log.i(TAG, "cameraInfo.facing == cameraFacing");
     ...
     * 延迟1.5秒后开始识别
     */
    public void startSpot(Boolean isCamera) {
        mSpotAble = true;
        if (isCamera==true){
            Log.i(TAG, "startSpot: startCamera");
            startCamera();
        }else{
            Log.i(TAG, "startSpot: startfrontCamera");
            startfrontCamera();
        }
        // 开始前先移除之前的任务
